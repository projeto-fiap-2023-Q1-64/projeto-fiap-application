        ...
        public void test() throws Exception {
                ...
                assertTrue(pedido.getItens().stream()
                                .allMatch(x -> produtos.stream()
                                                .anyMatch(y -> y.getCodigo()
                                                                .equals(x.getCodigo().getProdutoCodigo().toString()))));

                // Altera status do pedido para recebido - @PatchMapping("/{codigo}/pagar")
                pedido = restTemplate.patchForObject(createUriWithPort("/pedidos/" + codigoPedido + "/pagar"), null,
                                PedidoDTO.class);
                assertNotNull(pedido);
                ...
                restTemplate.exchange(createUriWithPort("/pagamento/gateway/url-do-gateway"), HttpMethod.POST,
                                httpEntity,
                                Void.class);

                // Recupera pagamentos aprovados - /por-status/{status}
                pagamentosAprovados = restTemplate
                                .exchange(createUriWithPort("/pagamentos/por-status/" + StatusPagamento.IN_PROCESS),
                                                HttpMethod.GET,
                                                null, new ParameterizedTypeReference<CustomPageImpl<PagamentoDTO>>() {
                                                })
                                .getBody();
                ...
                restTemplate.patchForObject(
                                createUriWithPort("/pagamentos/atualiza-pagamento/"
                                                + pagamentoStatusDTO.getCodigo().toString()),
                                httpEntity, Void.class);

                // Valida se o status alterou para aprovado
                pagamentosAprovados = restTemplate
                                .exchange(createUriWithPort("/pagamentos/por-status/" + StatusPagamento.APPROVED),
                                                HttpMethod.GET, null,
                                                new ParameterizedTypeReference<CustomPageImpl<PagamentoDTO>>() {
                                                })
                                .getBody();
                assertNotNull(pagamentosAprovados);
                ...
                pedido = restTemplate.exchange(createUriWithPort("/pedidos/" + codigoPedido + "/verificar-pagamento"),
                ...
                comandasPendentes = restTemplate
                                .exchange(createUriWithPort("/comandas/busca-pendentes"), HttpMethod.GET, null,
                ...
                pedido = restTemplate
                                .exchange(createUriWithPort("/pedidos/" + codigoPedido + "/entregar"), HttpMethod.PATCH,
